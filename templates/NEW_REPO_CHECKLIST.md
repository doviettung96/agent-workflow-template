...
1. Install `br` and Python on the machine.
...
6. Ensure early worker-targeted beads include `Read:`, `Inputs:`, `Files:`, and `Verify:` so a fresh worker can execute them without replaying planner chat.
...
6. Prefer worker-backed `executor-once` for bead-by-bead work; use `executor-loop` only as the current-session manual compatibility path.
