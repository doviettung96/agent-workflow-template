...
Drive an epic to completion while keeping `br`/`.beads` as the single source of truth and avoiding split-brain state between workers.
...
6. Initialize or refresh local `.beads/workflow/` in this worktree:
...
7. Inspect ready descendants:
   ```bash
   br ready --parent <epic-id> --json
   ```
8. Initialize shared Agent Mail and acquire the epic lock for this coordinator:
   - Windows:
     ```powershell
...
12. For each assignment, post a thread message so other sessions can inspect coordinator intent:
...
13. When Agent Mail is unavailable:
...
15. When the epic has no ready descendants left:
