...
6. Confirm the worktree resolves the main checkout’s Beads database:
   ```bash
   bd where
   ```
...
- Treat the worktree helper or native `bd worktree create` as the only supported entrypoint for epic swarm branches.
