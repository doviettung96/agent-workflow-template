...
5. Claim the bead and run the executor workflow â€” **every step in order**:
   - `beads-claim`
   - `writing-plans`
   - implementation
   - `systematic-debugging` if blocked
   - **`build-and-test`** â€” REQUIRED after implementation. Read the skill at `.codex/skills/build-and-test/SKILL.md` and follow it. Do NOT skip this step.
...
- If this executor run happens in a worktree, make sure it came from `start-epic-worktree` or native `bd worktree create`.
- If `bd where` fails in the worktree, stop and recreate the worktree through Beads or repair the main checkout with `bd bootstrap --yes`.
